...
The user-written package for creating raincloud plots ([`raincloudplots`](https://github.com/jorvlan/raincloudplots)), or they can be built using the geoms from the [`ggbeeswarm`](https://github.com/eclarke/ggbeeswarm), [`ggdist`](https://mjskay.github.io/ggdist/), and [`gghalves` packages](https://erocoar.github.io/gghalves/).
...
## `geom_boxplot`, `ggdist::stat_halfeye()` & `geom_point`
...
First we create a box-plot with `geom_boxplot()`, but include notches and removing the outliers. 
...
We add a `ggdist::stat_halfeye()` layer, mapping `species` to `fill`, and adjusting the size and shape of the density curve and moving it vertically above the box-plot.
...
::: {style="font-size: 0.75em;"}
The final layer is a `geom_point()`, mapping `fill` to `species` and setting `position` to `position_jitter()`. 

- Using `shape = 21`, we can `color` the outside of the point (white makes it appear to glow). 

- The `height` refers to the vertical area for the points 

- We also remove the legend for the entire graph with `theme(legend.position = "none")`
