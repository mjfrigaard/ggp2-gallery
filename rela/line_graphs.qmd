...
::: {style="font-size: 0.85em;"}
In the previous graph, we can see the number of births begins to decline around `2007` or `2008`.  We'll create another summarized dataset from the `usbirth_2010_2014` data, but this time we restrict the observations to only births in between `2007` and `2009`, group on `yr_qtr` and `day_of_week`, and get the average number of `births` (as `avg_births`).
...
Now when we create our line graph, we will have a categorical variable with more than two levels (`day_of_week`), so we should move the legend to the top of the graph using  `theme(legend.position = "top")` to improve readability. 

When we create the labels, we use `paste0()` to ensure the `subtitle` is accurate. 
...
label_dow <- usbirth_1994_2014 |> 
    ...
    dplyr::mutate(
        lbl = paste0(day, ", max births = ", max_births)) |> 
    dplyr::inner_join(y = usbirth_1994_2014, by = c("max_births" = "births")) |> 
    dplyr::select(day, max_births, lbl, date, day_category, yr_mnth, yr_qtr)
label_max_dow <- label_dow |> 
    head(1)
label_max_dow
label_min_dow <- label_dow |> 
    tail(1)
label_min_dow
```
:::
...
#| column: page-inset-right
#| layout-nrow: 1
labs_line_graph_grp_pal <- labs(title = "Average US births", 
                                subtitle = paste0("Between ",
...
::: {style="font-size: 0.85em;"}
Palettes are a great way to highlight or emphasize some lines over others. We'll start by making the lines for Monday, Thursday, and Friday subdued by adjusting the `alpha`.
:::

::: {style="font-size: 0.75em;"}
```{r}
#| label: ggp2_line_pal_1
#| code-fold: true
#| eval: true
#| echo: true
#| warning: false
...
library(paletteer)
library(ggthemes)
ggp2_line_pal_1 <- ggplot(data = dplyr::filter(avg_births_dow_mnth,
...
#| column: page-inset-right
#| layout-nrow: 1
ggp2_line_pal_2 <- geom_line(data = dplyr::filter(avg_births_dow_mnth, 
                                     day_of_week %in% c("Sat", "Sun")), 
                           aes(x = yr_mnth,
                               y = avg_births, 
                               group = day_of_week,
...
# layer 3
ggp2_line_pal_3 <- geom_line(data = dplyr::filter(avg_births_dow_mnth, 
...
ggp2_line_pal_4 <- geom_point(data = filter(label_max_dow, 
            day %in% c("Tues", "Wed")),
        aes(x = yr_mnth, 
            y = max_births, 
            color = day), 
...
# point for max births/day
# labels
ggp2_line_pal_5 <- geom_label(data = filter(label_max_dow, 
            day %in% c("Tues", "Wed")),
        aes(x = yr_mnth, 
            y = max_births, 
            label = lbl, 
...
ggp2_line_pal_1 +
    ...
    ggplot2::scale_color_manual(
        breaks = lev_order,
        values = paletteer::paletteer_d(palette = "ggthemes::gdoc", 
            n = 7)) +
    # add labels
    labs_line_graph_grp_pal +
    # move legend to top
...
med_births_dcat_dow_mnth <- usbirth_1994_2014 |>
    dplyr::filter(date >= lubridate::as_date("2009-04-01") &
