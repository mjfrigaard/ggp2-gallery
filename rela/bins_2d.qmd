...
In the previous example we showed how to reverse the fill color scale with the `rev` argument. Below we display how to reverse the color scale and manually set which colors on the scale we want to `begin` with (i.e., smallest data value) and which color we want to `end` with (i.e., the largest data value). Possible values range from `0` - `1`.

We also add a `geom_point()` layer, but set the `shape` to `21` (which gives us manual control over the point `color` and `fill`) . 
:::
