...
Grouped bar-graphs display calculated statistical measures for a numerical variable across the levels of a second categorical variable. Color is used to make comparisons and distinguish between groups (or levels) of the categorical variable. 

In `ggplot2`, we can create grouped bar-graphs with two geoms: `geom_bar()` and `geom_col()`
  ...
  - `geom_col()` will map *both* `x` *and* `y` aesthetics, and is used when we want to display numerical/quantitative values across the levels of a categorical variable, but these values have been created in their own column (see  [`stat_identity()`](https://ggplot2.tidyverse.org/reference/stat_identity.html) for more info)
