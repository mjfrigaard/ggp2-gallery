...
Typically, the graph contains two points representing the numerical value on the `y` axis, differentiated by color. A line connects the two points together, representing the difference (or ‘delta’) between the two values. The length of the line is the size of the difference. 
