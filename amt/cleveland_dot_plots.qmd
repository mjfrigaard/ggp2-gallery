...
::: {style="font-size: 0.85em;"}
Remove missing values from `sex` and `flipper_length_mm` and group on `sex` and `island` to the calculate the median flipper length, median bill length, and median bill depth. These variables are going to get 'showtime-ready' names because they'll be used in our facets. 
After ungrouping the data, pivot the new columns into a long (tidy) format with `median_measure` containing the name of the variable, and `median_value` containing the numbers. 
